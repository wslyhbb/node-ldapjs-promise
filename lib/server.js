...
const {
    ...
    CompareResponse,
    DeleteResponse,
    ExtensionResponse: ExtendedResponse,
    ModifyResponse,
    ModifyDnResponse,
    SearchRequest
} = messages;
...
function getResponse(req) {
    ...
    switch (req.protocolOp) {
        ...
        case Protocol.operations.LDAP_REQ_UNBIND:
            // TODO: when the server receives an unbind request this made up
...
class Server extends EventEmitter {
    ...
    constructor(options) {
        ...
        if ((options.cert || options.certificate) && options.key) {
            ...
            this.server = tls.createServer(options, options.connectionRouter ?
                options.connectionRouter : self.newConnection.bind(self))
        } else {
            this.server = net.createServer(options.connectionRouter ?
                options.connectionRouter : self.newConnection.bind(self))
        }
    ...
    newConnection(conn) {
        const self = this;
        const log = this.log;
        const options = this.server.ldap.config;
        ...
        conn.parser.on('message', function (req) {
            ...
            const chain = self.#getHandlerChain(req, res);
            ...
            return (function messageIIFE(err) {
                ...
                function after() {
                    if (!self.#postChain || !self.#postChain.length) { return; }

                    function next() { } // stub out next for the post chain

                    self.#postChain.forEach(function (cb) {
                        cb.call(self, req, res, next);
                    });
                }

                if (err) {
                    log.trace('%s sending error: %s', req.logId,
                        err.stack || err);
                ...
                try {
                    ...
                    if (req.protocolOp === Protocol.operations.LDAP_REQ_BIND &&
                        ...
                        if (req.dn.length === 0 && req.credentials === '') {
                            conn.ldap.bindDN = new DN({
                                rdns: [new RDN({ cn: 'anonymous' })]
                            });
                        } else {
                    ...
                    if (req.protocolOp === Protocol.operations.LDAP_REQ_UNBIND &&
                        res.status === 0) {
                        conn.ldap.bindDN = new DN({
                            rdns: [new RDN({ cn: 'anonymous' })]
                        });
    ...
    add(name) {
        ...
        return this.#mount(Protocol.operations.LDAP_REQ_ADD, name, args);
    }
    /**
    ...
    bind(name) {
        const args = Array.from(arguments).slice(1);
        return this.#mount(Protocol.operations.LDAP_REQ_BIND, name, args);
    }
    /**
    ...
    compare(name) {
        const args = Array.from(arguments).slice(1);
        return this.#mount(Protocol.operations.LDAP_REQ_COMPARE, name, args);
    }
    /**
    ...
    del(name) {
        const args = Array.from(arguments).slice(1);
        return this.#mount(Protocol.operations.LDAP_REQ_DELETE, name, args);
    }
    /**
    ...
    exop(name) {
        const args = Array.from(arguments).slice(1);
        return this.#mount(Protocol.operations.LDAP_REQ_EXTENSION, name, args, true);
    }
    /**
    ...
    modify(name) {
        const args = Array.from(arguments).slice(1);
        return this.#mount(Protocol.operations.LDAP_REQ_MODIFY, name, args);
    }
    /**
    ...
    modifyDN(name) {
        const args = Array.from(arguments).slice(1);
        return this.#mount(Protocol.operations.LDAP_REQ_MODRDN, name, args);
    }
    /**
    ...
    search(name) {
        const args = Array.from(arguments).slice(1);
        return this.#mount(Protocol.operations.LDAP_REQ_SEARCH, name, args);
    }
    /**
     ...
     */
    unbind() {
        const args = Array.from(arguments).slice(0);
        return this.#mount(Protocol.operations.LDAP_REQ_UNBIND, 'unbind', args, true);
    }
    use() {
    ...
    after() {
        if (!this.#postChain) { this.#postChain = []; }

        const self = this;
        mergeFunctionArgs(arguments).forEach(function (h) {
            self.#postChain.push(h);
        });
    }
    ...
    #setupConnection(c) {
        ...
        if (c.type === 'unix') {
            c.remoteAddress = this.server.path;
            c.remotePort = c.fd;
        } else if (c.socket) {
            // TLS
    ...
    #sortedRouteKeys() {
        ...
        if (!this.#routeKeyCache) {
            ...
            this.#routeKeyCache = output;
        }
        return this.#routeKeyCache;
    }
    #getHandlerChain(req) {
        assert.ok(req);
        ...
        const keys = this.#sortedRouteKeys()
    ...
    #mount(op, name, argv, notDN) {
        ...
        const route = this.#getRoute(notDN ? name : DN.fromString(name), backend);
