 ...
 */
module.exports = function enqueue(request) {
    ...
    // time has been reached, reject new entries until the queue has been
    // cleared.
    ...
    async function queueTimeout() {
        this.freeze();
        const timedOut = await this.purge();
        for (const { request, error } of timedOut) {
            request.cb(error);
        }
    }
};