...
class Client extends EventEmitter {
    ...
    abandon(messageId, controls) {
        ...
        return new Promise((resolve, reject) => {
            this._send(req, 'abandon', null, error => {
                if (error) {
    ...
    add(name, entry, controls) {
        ...
        return new Promise((resolve, reject) => {
            this._send(req, [errors.LDAP_SUCCESS], null, (error, res) => {
                if (error) {
     ...
     * @throws {TypeError} on invalid input.
     */
    bind(name, credentials, controls, _bypass) {
        if (
            typeof (name) !== 'string' &&
        ...
        return new Promise((resolve, reject) => {
            // Connection errors will be reported too (useful when the LDAP
            // server is not available)
            const self = this;
            function callbackWrapper(err, ret) {
                self.removeListener('connectError', callbackWrapper);
                if (err) {
                    self.log.trace('bind error: %s', err.message);
                    return reject(err);
                }

                self.log.trace('bind successful for user: %s', name);
                return resolve(ret);
            }
    ...
    compare(name, attr, value, controls) {
        ...
        return new Promise((resolve, reject) => {
            this._send(req, CMP_EXPECT, null, (error, res) => {
                if (error) {
                ...
                return resolve({
                    matched: (res.status === errors.LDAP_COMPARE_TRUE),
                    response: res
                });
            });
        });
    ...
    unbind() {
        ...
        return new Promise((resolve, reject) => {
            this._send(req, 'unbind', null, error => {
                if (error) {
