 ...
 *                able to emit 'end'.
 */
class SearchPager extends CorkedEmitter {
    constructor(opts) {
        assert.object(opts);
        ...
        emitter.on('end', this.#onEnd.bind(this));
    ...
    #onEnd(res) {
        ...
        if (cookie === null) {
            ...
            if (this.listeners('pageError').length > 0) {
                ...
                // If the consumer as subscribed to pageError, SearchPager is
                // absolved from delivering the fault via the 'error' event.
                // Emitting an 'end' event after 'error' breaks the contract
                // that the standard client provides, so it is only a
                // possibility if 'pageError' is used instead.
                this.emit('end', res);
            } else {
                this.emit('error', err);
    ...
    #nextPage(cookie) {
        ...
        this.sendRequest(controls, this.childEmitter, this.#sendCallback.bind(this));
    }
