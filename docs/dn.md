---
title: DN API | ldapjs-promise
---

# ldapjs-promise DN API

<div class="intro">

This document covers the ldapjs-promise DN API and assumes that you are
...
The `parseDN` API converts a string representation of a DN into an
ldapjs-promise DN object; in most cases this will be handled for you under the
covers of the ldapjs-promise framework, but if you need it, it is there.
...
The DN object is largely what you will be interacting with, since all the
server APIs are setup to give you a DN object.
...
The inverse of `childOf`; returns a boolean on whether or not `this` is a
parent of the passed in dn.  Like `childOf`, can take either a string or a DN.
