---
title: Client API | ldapjs-promise
---

# ldapjs-promise Client API

<div class="intro">

This document covers the ldapjs-promise client API and assumes that you are
familiar with LDAP. If you are not, read the [guide](guide.html) first.
...
The client is an `EventEmitter`. If you do not register an error handler and
...
The last parameter in every API is `controls`. `controls` can be either a
single instance of a `Control` or an array of `Control` objects.
You can, and probably will, omit this option.

Almost every operation returns a promise that either resolves a `res` or throws
an `err` where err will be an instance of an `LDAPError` (you can use
`instanceof` to switch).
You probably will not need to check the `res` parameter, but it is there if you do.
...
of `Control` objects. You probably do not need them though...
...
```js
const res = await client.compare('cn=foo, o=example', 'sn', 'bar');
...
going to be an OID (well, the RFC says it must be; however, ldapjs-promise has
no such restriction).  `value` is completely arbitrary, and is whatever the
exop says it should be.
...
A `Change` object maps to the LDAP protocol of a modify change, and requires
you to set the `operation` and `modification`.  The `operation` is a string,
and must be one of:
...
* There is no ability to set "keep old dn."  It is always going to flag the old
dn to be purged.
...
takes an `options` object for all the parameters.  However, ldapjs-promise
makes some defaults for you so that if you pass nothing in, it is pretty much
equivalent to an HTTP GET operation (i.e., base search against the DN, filter
set to always match).
...
Responses returned by the `search` method are an `EventEmitter` where you will
get a notification for each `searchEntry` that comes back from the server. You
will additionally be able to listen for a `searchRequest`, `searchReference`,
`error` and `end` event.
`searchRequest` is emitted immediately after every `SearchRequest` is sent with
a `SearchRequest` parameter. You can do operations like `client.abandon` with
`searchRequest.messageId` to abandon this search request. Note that the `error`
event will only be for client/TCP errors, not LDAP error codes like the other
APIs. You will want to check the LDAP status code (likely for `0`) on the `end`
event to assert success. LDAP search results can give you a lot of status
codes, such as time or size exceeded, busy, inappropriate matching, etc., which
is why this method does not try to wrap up the code matching.
...
const res = await client.search('o=example', opts);
...
ldapjs-promise does not support extensible matching, since it is one of those
features that almost nobody actually uses in practice.

Assuming you do not really want to read the RFC, search filters in LDAP are
basically a "tree" of attribute/value assertions, with the tree specified in
prefix notation.  For example, let us start simple, and build up a complicated
filter.  The most basic filter is equality, so let us assume you want to search
...
ldapjs-promise requires all filters to be surrounded by '()' blocks. Ok, that
was easy. Let us now assume that you want to find all records where the email
is actually just anything in the "@bar.com" domain and the location attribute
is set to Seattle:
...
`(email=*@*bar.com)` to match any email of @bar.com or its subdomains like
`"example@foo.bar.com"`.

Now, let us say we also want to set our filter to include a specification that
either the employeeType *not* be a manager nor a secretary:
...
It gets a little bit complicated, but it is actually quite powerful, and lets
...
const res = await client.search('o=largedir', opts);
res.on('searchEntry', (entry) => {
...
const res = await client.search('o=largerdir', opts);
res.on('searchEntry', (entry) => {
...
# searchReturnAll
`searchReturnAll(base, options, controls)`

Performs a search operation against the LDAP server.

A wrapper method around the search method that collects all of the entries and
referrals from the search and returns it as an object. An opinionated function
that simply returns the results but lacks the control of the search method.
...
const res = await client.searchReturnAll('o=example', opts);
...
Note that unbind operation is not an opposite operation for bind. Unbinding
results in disconnecting the client regardless of whether a bind operation was
performed.
