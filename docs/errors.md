---
title: Errors API | ldapjs-promise
---

# ldapjs-promise Errors API

<div class="intro">

This document covers the ldapjs-promise errors API and assumes that you are
familiar with LDAP. If you are not, read the [guide](guide.html) first.

</div>

All errors in the ldapjs-promise framework extend from an abstract error type
called `LDAPError`. In addition to the properties listed below, all errors will
have a `stack` property correctly set.

In general, you will be using the errors in ldapjs-promise like:
...
I.e., if you just pass them into the `next()` handler, ldapjs-promise will
automatically return the appropriate LDAP error message, and stop the handler
chain.
