---
title: Server API | ldapjs-promise
---

# ldapjs-promise Server API

<div class="intro">

This document covers the ldapjs-promise server API and assumes that you are
...
Internally, [abstract-logging](https://www.npmjs.com/packages/abstract-logging)
is used to implement the interface.  As a result, no log messages will be
generated unless an external logger is supplied.
...
The LDAP server API mirrors the [Node.js `server.getConnections` API](https://nodejs.org/dist/latest-v22.x/docs/api/net.html#net_server_getconnections_callback). Callback
...
Note that ldapjs-promise is also slightly different, since it is often going to
be backed to a DB-like entity, in that it also has an API where you can pass in
a 'backend' object.  This is necessary if there are persistent connection pools,
...
The first parameter to an ldapjs-promise route is always the point in the
...
signature (ldapjs-promise will unroll them).
...
However, it is more common/easier to use the `return next(new LDAPError())`
pattern, since ldapjs-promise will fill in the extra LDAPResult fields like
matchedDN and error message for you.

## Errors

ldapjs-promise includes an exception hierarchy that directly corresponds to the
RFC list of error codes.  The complete list is documented in
[errors](errors.html).  But the paradigm is something defined like
CONSTRAINT\_VIOLATION in the RFC would be `ConstraintViolationError` in
ldapjs-promise.  Upon calling `next(new LDAPError())`, ldapjs-promise will
_stop_ calling your handler chain.  For example:
...
objects (which have a type and an array of values).  This directly map to how
the request came in off the wire.  It is likely you will want to use
...
The number of entries to return. Defaults to '0' (unlimited). ldapjs-promise
does not currently automatically enforce this, but probably will at some point.
...
'false'.  ldapjs-promise will take care of this for you.
...
An optional list of attributes to restrict the returned result sets to.
ldapjs-promise will automatically handle this for you.
...
OID, but ldapjs-promise makes no such restrictions; it just needs to be a
string.  Unlike the other operations, extended operations don't map to any
location in the tree, so routing here will be exact match, as opposed to
subtree.
...
ldapjs-promise by default provides an unbind handler that just disconnects the
client and cleans up any internals (in ldapjs-promise core).  You can override
this handler if you need to clean up any items in your backend, or perform any
other cleanup tasks you need to.
...
Note that the LDAP unbind operation actually does not send any response (by
