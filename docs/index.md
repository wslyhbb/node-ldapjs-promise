...
ldapjs-promise is a pure JavaScript, from-scratch framework for implementing
...
ldapjs-promise implements most of the common operations in the LDAP v3 RFC(s),
for both client and server.  It is 100% wire-compatible with the LDAP protocol
itself, and is interoperable with [OpenLDAP](http://openldap.org) and any other
LDAPv3-compliant implementation.  ldapjs-promise gives you a powerful routing
and "intercepting filter" pattern for implementing server(s).  It is intended
...
If you are new to LDAP, check out the [guide](guide.html).  Otherwise, the
...
- Code: [wslyhbb/node-ldapjs-promise](https://github.com/wslyhbb/node-ldapjs-promise)
